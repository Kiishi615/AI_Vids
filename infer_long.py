...
def main():
    ...
    with torch.no_grad():
        ...
        print(f"Audio length: {int(len(mel_input)/ sr * fps)}, Video length: {video_length_actual}")
        audio_feature_wav2vec = get_audio_embed(mel_input, wav2vec_feature_extractor, audio_encoder, video_length_actual, sr=16000, fps=fps, device='cpu')
        ...
        audio_embeds = audio_embeds[center_indices] # [F, 5, 12, 768]
        ...
        while init_frames < video_length_actual:
            ...
            if init_frames != 0:
                mix_ratio = torch.from_numpy(
                    np.array([float(i) / float(overlap_video_length) for i in range(overlap_video_length)], np.float32)
                ).unsqueeze(0).unsqueeze(0).unsqueeze(-1).unsqueeze(-1).to(device=sample.device)
